...
public class MainActivity extends Activity {
    private static final String PREFS = "jule_omega_spaces_v2";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lastOmega = Math.max(DEFAULT_LAST_OMEGA,
                prefs.getInt(KEY_LAST_OMEGA, DEFAULT_LAST_OMEGA));
        ...
        renderOmegaButtons();
        loadUrl(prefs.getString(omegaKey(0), DEFAULT_HOME));
    }
    ...
    private void renderOmegaButtons() {
        ...
        for (int index = 0; index <= lastOmega; index++) {
            final int omegaIndex = index;
            Button button = new Button(this);
            button.setText("Ω" + omegaIndex);
            ...
            button.setOnClickListener(v -> openOmega(omegaIndex));
            button.setOnLongClickListener(v -> { saveCurrentPageToOmega(omegaIndex); return true; });
            omegaContainer.addView(button);
        }
    }

    private void openOmega(int omegaIndex) {
        String savedUrl = prefs.getString(omegaKey(omegaIndex), null);
        if (savedUrl == null || savedUrl.trim().isEmpty()) {
            saveCurrentPageToOmega(omegaIndex);
            return;
        }
        loadUrl(savedUrl);
    }

    private void saveCurrentPageToOmega(int omegaIndex) {
        ...
        if (currentUrl == null || currentUrl.trim().isEmpty()) {
            ...
            currentUrl = normalize(entered);
        }

        prefs.edit().putString(omegaKey(omegaIndex), currentUrl).apply();
        Toast.makeText(this, "Saved current page to Ω" + omegaIndex, Toast.LENGTH_SHORT).show();
    }

    private void addOmega() {
        lastOmega++;
        prefs.edit().putInt(KEY_LAST_OMEGA, lastOmega).apply();
        renderOmegaButtons();
        Toast.makeText(this, "Ω" + lastOmega + " created", Toast.LENGTH_SHORT).show();
    }

    private String omegaKey(int omegaIndex) { return "omega_" + omegaIndex; }

    private void hideKeyboard() {
        InputMethodManager keyboard = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
        if (keyboard != null) keyboard.hideSoftInputFromWindow(addressBar.getWindowToken(), 0);
